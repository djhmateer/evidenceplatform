...
Username recycling invariant: Instagram can recycle usernames — two distinct pks can legitimately hold the same `url_suffix` at different times. The system enforces that two accounts with different non-null `id_on_platform` values are **always** separate rows: (1) `UNIQUE(id_on_platform)` at the DB level (V038), (2) `merge_account_into` raises `ValueError` when both parties hold a pk, and (3) `auto_merge_shadowed_stubs` only folds pk-less stubs, never two pk-holders. Affiliations established via username-only sessions can be audited via `GET /account/{id}/attribution-report`, which checks whether each affiliated entity (comment, like, tag, post) was ever observed with the account's pk — if not, the attribution is ambiguous when another account holds the same username.
