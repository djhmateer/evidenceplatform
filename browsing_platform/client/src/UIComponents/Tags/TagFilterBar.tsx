...
import {SCOPE_OPTIONS, resolveScopes, isDefaultScopes} from "../../lib/tagScopes";
...
export default function TagFilterBar({tagIds, tagFilterMode, selectedTagObjects, tagScopes, onChange, entity}: IProps) {
    ...
    const fetchOptions = async (value: string) => {
        ...
        if (seq !== fetchSeq.current) return; // a newer keystroke superseded this request
        setOptions(results);
        setFetching(false);
    };

    // Single funnel for every change: re-emit current state with only the given fields overridden.
    // tagIds and tagObjects are kept independent — the id list stays authoritative even when its
    // tag objects haven't been resolved yet, so mode/scope changes never drop unresolved ids.
    const emit = (changes: {
        tagIds?: number[]; mode?: "any" | "all"; tagObjects?: ITagWithType[]; scopes?: E_ENTITY_TYPES[];
    }) => onChange(
        changes.tagIds ?? tagIds,
        changes.mode ?? tagFilterMode,
        changes.tagObjects ?? selectedTagObjects,
        changes.scopes ?? tagScopes,
    );

    const handleTagsChange = (newTags: ITagWithType[]) => emit({tagIds: newTags.map(t => t.id), tagObjects: newTags});

    const handleModeChange = (_: React.MouseEvent, newMode: "any" | "all" | null) => {
        if (newMode) emit({mode: newMode});
    };
    ...
    const handleQuickAccessSelect = (tag: ITagWithType) => {
        ...
        emit({tagIds: newTags.map(t => t.id), tagObjects: newTags});
    };
    ...
    const isBroadened = selfScope ? !isDefaultScopes(tagScopes, selfScope) : false;
